...
function wcTalkingPoint(c: ClubEntry, rank: number, grandTotal: number): string {
  ...
  if (c.hatTricks >= 2)
    return `${c.club} players have delivered ${c.hatTricks} hat-tricks — the most explosive club presence at cup26.`;
  if (c.hatTricks === 1)
    return `${c.club} already has a hat-trick at cup26 — ${top2} carrying the banner for ${c.league}.`;
  ...
  if (c.wcGoals === 0 && c.wcMinutes > 0)
    return `${c.club}'s ${c.playerNames.length} players have clocked ${c.wcMinutes} cup26 minutes — contributing without getting on the scoresheet.`;
...
function previewTalkingPoint(c: ClubEntry, rank: number): string {
  ...
  if (c.careerGoals > 30)
    return `${c.playerNames.length} ${c.club} players bring a combined ${c.careerGoals} international goals to cup26 — elite firepower on the stage.`;
...
function leagueTalkingPoint(l: LeagueEntry, rank: number, grandTotal: number, isLive: boolean): string {
  ...
  if (isLive) {
    if (rank === 0 && l.goals > 0)
      return `${l.league} clubs account for ${pct}% of all cup26 goals — the clearest argument for its global supremacy.`;
    if (l.hatTricks > 0)
      return `${l.league} players have recorded ${l.hatTricks} hat-trick${l.hatTricks > 1 ? "s" : ""} — elite finishing runs in this league.`;
    return `${l.clubs} ${l.league} clubs: ${l.goals}G ${l.assists}A combined at cup26.`;
  } else {
    if (rank === 0)
      return `${l.league} sends the most players to this cup26 — ${l.players} players from ${l.clubs} clubs bringing ${l.careerGoals} combined international goals.`;
    if (l.careerGoals > 50)
    ...
    return `${l.clubs} ${l.league} clubs represented across ${l.players} cup26 players.`;
  }
}
